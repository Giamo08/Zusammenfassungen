...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Ideal FIR Filters}

The ideal frequency response $D(\omega)$ and its impulse response
$d(k)$ are related by the inverse DTFT

\[
d(k)=\frac{1}{2\pi}
\int_{-\pi}^{\pi}
D(\omega)e^{j\omega k}\,d\omega.
\]
...
d(k)
&=
\frac{\sin(\omega_bk)-\sin(\omega_ak)}
{\pi k}
\\[1ex]
\text{Bandstop:}\qquad
d(k)
...
the limit has to be evaluated separately.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Symmetric and Antisymmetric FIR Filters}

The impulse response determines the phase behaviour of the FIR filter.

\paragraph{Symmetric filters}
...
The frequency response is real and even.

\vspace{0.5em}

\paragraph{Antisymmetric filters}
...
\begin{itemize}
\item Differentiator
\item Hilbert Transformer
\end{itemize}

The frequency response is imaginary and odd.

\vspace{0.5em}

Both classes have linear phase.
\paragraph{Complementary Filters}

The following ideal filters are complementary:
...
\begin{itemize}
\item Overshoot $\approx 8.9\%$
\item Does not disappear for larger $N$
\item Only the transition region becomes narrower
\end{itemize}
...
\begin{itemize}
\item much smaller passband and stopband ripple
...
The cutoff frequency is chosen halfway between the passband and
stopband edges:
...
For a real-time FIR implementation
...
The corresponding minimum transition width is
...
For a fixed DSP instruction rate
...
Once the impulse response coefficients have been determined,
the FIR filter is implemented by
...
\begin{itemize}
\item $h(m)$ are the FIR coefficients,
\item $N$ is the filter length.
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Group Delay}

A linear-phase FIR filter of length

\[
N
\]

has the constant group delay
...
The corresponding delay in seconds is
