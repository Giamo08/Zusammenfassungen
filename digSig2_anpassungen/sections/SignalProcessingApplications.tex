...
\paragraph{Example (Initialization)}

For

\[
H(z)=\frac{z^{-1}+2z^{-2}}{1-z^{-3}}
\]

the canonical realization is initialized by exciting the filter with
a unit impulse $x(0)=1$.
...
The sequence stored in the recursive delay line repeats every
$D=3$ samples and therefore generates the periodic waveform.
...
\subsubsection{Fractional Wavetable Readout}

If the read pointer is not an integer, the neighbouring samples must be
used to determine the output value.
...
where $i=\lfloor p\rfloor$ is the integer part and $\alpha$ the
fractional part.

The most common readout methods are:

\paragraph{Truncate Down}
...
\paragraph{Truncate Up}

Always use the upper index.
...
Use a weighted average of the neighbouring samples.

\[
\boxed{
y=(1-\alpha)w_i+\alpha w_{i+1}
}
\]

Linear interpolation provides the smoothest waveform and therefore
introduces the smallest distortion.
...
\[
\begin{array}{c|c|c}
\textbf{Method} & \textbf{Formula} & \textbf{Output}\\
\hline
\text{Truncate Down}
&
y=w_i
...
\paragraph{Transfer Function from the Difference Equation}

The transfer function is obtained by taking the $z$-transform of the
difference equation assuming zero initial conditions.
...
The quality factor $Q$ describes how narrow the notch is.
...
A larger $Q$ results in a narrower notch and higher frequency
selectivity.

For periodic comb filters, the parameter $a$ can be calculated from the
quality factor as

\[
\boxed{
a=
\frac{1-\tan\left(\frac{\pi}{2Q}\right)}
{1+\tan\left(\frac{\pi}{2Q}\right)}
}
\]

A larger value of $a$ produces a narrower comb response.
...
\paragraph{Circular Buffer Implementation}

Instead of shifting all delay elements, a circular buffer updates only
one memory location per sample.

The circular pointer is updated according to

\[
q=(q+1)\bmod(D+1)
\]

The delayed sample is accessed by
