...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Notation und verwendete Symbole}

In der digitalen Signalverarbeitung werden in allen Kapiteln dieselben
Symbole verwendet. Die wichtigsten Grössen sind im Folgenden
zusammengefasst.
...
\begin{center}
	...
	\begin{tabularx}{\columnwidth}{|c|X|}
		...
		$f$ & Frequenz eines Signals in Hertz (Hz). \\ \hline
		$f_s$ & Abtastfrequenz (Sampling Frequency). Gibt an, wie viele Samples pro Sekunde aufgenommen werden. Einheit: Hz. \\ \hline
		$T=\frac{1}{f_s}$ & Abtastperiode (Sampling Period). Zeit zwischen zwei aufeinanderfolgenden Samples. Einheit: Sekunden. \\ \hline
		$f_0$ & Grundfrequenz bzw. gewünschte Frequenz eines Sinussignals. \\ \hline
		$\omega$ & Digitale Kreisfrequenz in Radiant pro Sample. Es gilt $\omega = 2\pi\frac{f}{f_s}$ und der Nyquistbereich ist $-\pi \le \omega \le \pi$. \\ \hline
		$\omega_0$ & Digitale Kreisfrequenz der Grundfrequenz $f_0$: $\omega_0=2\pi\frac{f_0}{f_s}$. \\ \hline
		$T_D$ & Dauer einer Periode eines periodischen Signals mit $T_D=\frac{1}{f}$. \\ \hline
		$D$ & Anzahl Samples innerhalb einer Periode eines periodischen Signals. Oft auch Interpolations- oder Dezimationsfaktor. \\ \hline
		$L$ & Interpolationsfaktor. Zwischen zwei Samples werden $L-1$ neue Samples eingefügt. Die neue Abtastfrequenz beträgt $f_s'=L\,f_s$. \\ \hline
		$M$ & Dezimationsfaktor. Es wird nur jedes $M$-te Sample übernommen. Die neue Abtastfrequenz beträgt $f_s'=\frac{f_s}{M}$. \\ \hline
	\end{tabularx}
\end{center}
...
\begin{center}
	...
	\begin{tabularx}{\columnwidth}{|c|X|}
		\hline
		\textbf{Symbol} & \textbf{Bedeutung} \\ \hline
		$n$ & Diskreter Zeitindex ($n=0,1,2,\ldots$). \\ \hline
		$N$ & Anzahl Samples eines Signals oder Länge einer DFT bzw. FFT. \\ \hline
		$x[n]$ & Eingangssignal im Zeitbereich. \\ \hline
		$y[n]$ & Ausgangssignal im Zeitbereich. \\ \hline
		$h[n]$ & Impulsantwort eines Filters. \\ \hline
		$H(z)$ & Übertragungsfunktion eines digitalen Filters im $z$-Bereich. \\ \hline
		$X[k]$ & DFT bzw. FFT des Signals. \\ \hline
		$R$ & Radius der Pole im $z$-Diagramm. Für $0<R<1$ ist das System stabil. Je näher $R$ an $1$ liegt, desto langsamer klingt das Signal ab. \\ \hline
		$z^{-1}$ & Verzögerung um genau ein Sample (Unit Delay). \\ \hline
	\end{tabularx}
\end{center}

\paragraph{Wichtige Zusammenhänge}
...
\paragraph{Einfluss des Fensters}

Ein Hann-Fenster reduziert Leakage,
verringert jedoch die Amplitude der Spektrallinien
und verbreitert die Hauptkeule.

\paragraph{Merke}
Die physikalische Frequenzauflösung wird durch die Messdauer bestimmt:

\[
\Delta f=\frac1T=\frac{f_s}{L}
\]

Die DFT besitzt dagegen eine Binbreite

\[
\Delta f_{\text{bin}}=\frac{f_s}{N}.
\]

Nur eine längere Messdauer verbessert die tatsächliche Frequenzauflösung.
Zero Padding erhöht lediglich die Anzahl der DFT-Punkte.
...
of the DTFT.
\paragraph{Known Sinusoids}

If

\[
x(n)=A\cos\left(2\pi\frac{k_0}{N}n\right),
\]

then only two FFT coefficients are nonzero:
...
Each coefficient has magnitude

\[
\frac{AN}{2}.
\]

A DC component appears only at
...
Ein Sinus mit der Frequenz \(f_0\) erscheint im DFT-Bin

\[
k=\frac{Nf_0}{f_s}.
\]

Für reelle Signale entsteht zusätzlich der Spiegelpeak bei

\[
N-k.
\]

Liegt \(k\) nicht auf einer ganzen Zahl, verteilt sich die Energie auf mehrere Bins
(Leakage).

\subsection{Interpretation der FFT}

Für ein reelles Signal gilt:

\[
k=\frac{Nf_0}{f_s}
\]

Der FFT-Bin \(k\) entspricht der Frequenz

\[
f_k=\frac{kf_s}{N}.
\]

Ein Cosinus erscheint immer als zwei Spektrallinien:
...
Ein Sinus besitzt dieselben FFT-Indizes, jedoch rein imaginäre
Spektralwerte.
...
\paragraph{Signals with the Same DFT}

Signals that differ only by multiples of the DFT length produce the same
DFT after modulo-\(N\) reduction.

Only the modulo-\(N\) wrapped sequence determines the DFT.
...
Die IFFT liefert wieder das Zeitsignal.

Reelle Spektren ergeben Cosinusanteile.

Rein imaginäre Spektren ergeben Sinusanteile.
...
\subsection{Overlap-Save Method}

The overlap-save method performs long convolutions efficiently by using
the FFT.
...
samples are copied from the previous block.

After multiplication in the frequency domain and the inverse FFT,
the first
...
output samples are discarded because they contain circular convolution
artifacts.
...
\paragraph{Relationship to Linear Convolution}

Circular convolution is obtained by reducing the linear convolution
modulo \(N\).
...
If both signals are zero-padded to at least
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Rechenaufwand von DFT-Algorithmen}

Für die Berechnung einer DFT gibt es verschiedene Algorithmen mit
unterschiedlichem Rechenaufwand. Da Multiplikationen wesentlich
aufwändiger sind als Additionen, wird meist die Anzahl der komplexen
Multiplikationen verglichen.

\subsubsection*{Direkte DFT}

Für jedes der $N$ Ausgabesamples werden $N$ Multiplikationen benötigt.

\[
\boxed{N_{\mathrm{mult}} = N^2}
\]
...
\subsubsection*{DFT mit Modulo-$N$-Reduktion}

Durch die Modulo-$N$-Eigenschaft werden identische Twiddle-Faktoren
zusammengefasst. Dadurch reduziert sich die Anzahl der Multiplikationen.

\[
\boxed{N_{\mathrm{mult}}
=
N\,(2N-1)}
\]

(Falls in den Übungen nach der Anzahl der Twiddle-Faktor-Multiplikationen gefragt wird.)

\vspace{0.5em}

\subsubsection*{FFT (Radix-2)}

Bei der FFT wird das Problem rekursiv halbiert.
...
Butterflies pro Stufe:

\[
\boxed{\frac{N}{2}}
\]

Jeder Butterfly benötigt genau eine komplexe Multiplikation.
...
\subsubsection*{FFT mit Modulo-$N$-Reduktion}

Durch Ausnutzen der Symmetrien der Twiddle-Faktoren können nochmals
...
Die genaue Anzahl hängt von den verwendeten Twiddle-Faktoren ab und wird
meist direkt aus dem Butterfly-Diagramm bestimmt.
...
\begin{array}{|c|c|}
\hline
\textbf{Verfahren} & \textbf{Komplexe Multiplikationen}\\
\hline
\text{DFT} & N^2\\
\hline
\text{DFT (Modulo)} & \text{abhängig von der Modulo-Reduktion}\\
\hline
\text{FFT} & \dfrac{N}{2}\log_2 N\\
\hline
\text{FFT (Modulo)} & < \dfrac{N}{2}\log_2 N\\
\hline
\end{array}
}
\]
